...
COMMAND="python $HOME/git_checkouts/agdc/api-examples/source/main/python/workflow/arg_stats.py --output-dir $outputdir --x-min $xmin --x-max $xmax --y-min $ymin --y-max $ymax --acq-min $acqmin --acq-max $acqmax --mask-pqa-apply --mask-wofs-apply --chunk-size-x $chunkx --chunk-size-y $chunky --add_on_name $add_on_name"
